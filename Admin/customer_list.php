            ...
            <div class="card-body">
              ...
              <table id="example" class="table table-bordered">
                ...
                <tbody>
                      ...
                      <?php 
                        $Customer_user=mysqli_query($conn,"SELECT * FROM tbl_user WHERE role_id=1");
                        while($fetch_customers=mysqli_fetch_array($Customer_user))
                        {
                       ?>
                       <tr>
                        <td><?php echo $fetch_customers['username']; ?></td>
                        <td><?php echo $fetch_customers['email']; ?></td>
                        <td><?php echo $fetch_customers['contact_no']; ?></td>
                        
                        <?php if($fetch_customers['state_id']==0)
                        {
                           echo '<td>INACTIVE</td>';
                        } elseif($fetch_customers['state_id']==1)
                        {
                            echo '<td>ACTIVE</td>';
                        }else{
                              echo '<td>DELETED</td>';
                        ...
                        if($fetch_customers['state_id']==0 || $fetch_customers['state_id']==2)
                        {
                          echo '<input type="hidden" name="cust_id" value='.$fetch_customers["id"].'><td><button class="btn btn-primary" name="change_status" value="0" type="submit">ACTIVE</button></td>';

                        }else{
                          echo '<input type="hidden" name="cust_id" value='.$fetch_customers["id"].'><td><button class="btn btn-secondary" name="change_status" value="1" type="submit">DELETE</button></td>';
                        }
                          echo '</form>

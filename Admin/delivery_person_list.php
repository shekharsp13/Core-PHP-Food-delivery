            ...
            <div class="card-body">
              ...
              <table id="example" class="table table-bordered">
                ...
                <tbody>
                      ...
                      <?php 
                        $delivery_person=mysqli_query($conn,"SELECT * FROM tbl_user WHERE role_id=2");
                        while($fetch_delivery=mysqli_fetch_array($delivery_person))
                        {
                       ?>
                       <tr>
                        <td><?php echo $fetch_delivery['username']; ?></td>
                        <td><?php echo $fetch_delivery['email']; ?></td>
                        <td><?php echo $fetch_delivery['contact_no']; ?></td>
                        
                        <?php if($fetch_delivery['state_id']==0)
                        {
                           echo '<td>INACTIVE</td>';
                        } elseif($fetch_delivery['state_id']==1)
                        {
                            echo '<td>ACTIVE</td>';
                        }else{
                              echo '<td>DELETED</td>';
                        ...
                        if($fetch_delivery['state_id']==0 || $fetch_delivery['state_id']==2)
                        {
                          echo '<input type="hidden" name="cust_id" value='.$fetch_delivery["id"].'><td><button class="btn btn-primary" name="change_status" value="0" type="submit">ACTIVE</button></td>';

                        }else{
                          echo '<input type="hidden" name="cust_id" value='.$fetch_delivery["id"].'><td><button class="btn btn-secondary" name="change_status" value="1" type="submit">DELETE</button></td>';
                        }
                          echo '</form>
